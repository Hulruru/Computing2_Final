/**
 데이터의 희소도를 런타임에 분석하여 [Dense Mode]와 [Sparse Mode]를 자동으로 전환
 */
...
#define TOLERANCE 1e-20         // 조기 종료를 위한 수렴 기준값

// Constant Memory: 중심점 좌표 및 Norm 캐싱
...
/**
 * @brief 두 중심점 벡터 사이의 변화량(L1 Norm)을 계산하여 수렴 여부 확인
 */
...
float run_dense_kmeans(const std::vector<float>& h_data_aos, int n_samples, int n_features, std::vector<float>& final_centroids) {
    ...
    for (int iter = 0; iter < MAX_ITER; ++iter) {
        CUDA_CHECK(cudaMemcpyToSymbol(c_centroids, d_centroids, centroid_size_float, 0, cudaMemcpyDeviceToDevice));
...
float run_csr_kmeans(const std::vector<float>& h_data_aos, int n_samples, int n_features, std::vector<float>& final_centroids) {
    ...
    cudaEventRecord(start);
    for (int iter = 0; iter < MAX_ITER; ++iter) {
        // 거리 계산 최적화를 위해 Centroid Norm 미리 계산
        std::vector<float> h_curr_centroids(N_CLUSTERS * n_features);
        ...
        h_prev_centroids = h_curr_centroids; // 업데이트
...
// ==========================================
int main() {
    printf("=== [Batch Adaptive CUDA K-Means (Verified: Early Stopping)] ===\n");
